...
class DrugInteractionWarning {
  ...
  bool get isSevere {
    final String normalized = severity.trim().toUpperCase();
...
class VietQrConfig {
  ...
  String buildTransferContent(String orderId) {
    final String cleanId = orderId
        ...
        .toUpperCase();
    final String suffix = cleanId.length > 18
        ? cleanId.substring(cleanId.length - 18)
        : cleanId;
    return 'HD$suffix';
  }
  ...
  String buildImageUrl({
    ...
    final String cleanBankId = bankId.trim();
    final String cleanAccountNo = accountNo.replaceAll(RegExp(r'\s+'), '');
    final String cleanTemplate = template.trim().isEmpty
        ? 'compact2'
        : template.trim();

    final Uri uri = Uri.https(
      'img.vietqr.io',
      '/image/$cleanBankId-$cleanAccountNo-$cleanTemplate.png',
      <String, String>{
